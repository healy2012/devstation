...
<div class="content"><!-- content start -->
   <h3 class="breadcrumb">
   <span class="current">您现在的位置：用户管理DEMO&gt;<a target="mainFrame" href="<%=basePath%>demo/portal/queryPortalUser.action">用户管理</a>&gt;<a target="mainFrame" href="<%=basePath%>demo/updateUser.action?userIds=${userInfo.userId}">修改用户</a></span>
  </h3>
   
  <h4 class="warning">请填写以下信息完成注册。加“*”为必填项。</h4>
  
  <s2:form id="reg" action="demo/updateSaveUser.action">
  <table class="table_form">
    <tr>
      <th>登录帐号：</th>
      <td><input name="userInfo.userName" id="username" type="text" value="${userInfo.userName}" class="required" maxlength="20" disabled/></td>
      <td class="place"><p class="annotation">登陆账号不能修改</p></td>
    </tr>
    <tr>
      <th>登录密码：</th>
      <td><input name="userInfo.password" id="password" type="password" value="${userInfo.password}" class="required" maxlength="20"/></td>
    ...
    <tr>
      <th>真实姓名：</th>
      <td><input name="userInfo.name" id="realname" type="text" class="required" value="${userInfo.name}"/></td>
      <td class="place"><p class="annotation">请填写您的真实姓名。只能使用汉字，且至少两个汉字</p></td>
    </tr>
    <tr>
      <th>E-mail：</th>
      <td><input name="userInfo.email" id="email" type="text" value="${userInfo.email}" class="required"/></td>
      <td class="place"><p class="annotation">请填写真实并且最常用的邮箱</p></td>
    </tr>
    <tr>
      <th>手机号码：</th>
      <td><input name="userInfo.tel" id="mobile" type="text" value="${userInfo.tel}" class="required"/></td>
