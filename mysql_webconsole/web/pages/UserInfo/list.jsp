...
<html xmlns=" http://www.w3.org/1999/xhtml">
  <head>
    ...
    {
     $(this).attr("href","<%=basePath%>pages/UserInfo/edit.action?items="+f_str);
    }
    ...
    else
    {
      ...
      if(flag)
      {
        $(this).attr("href","<%=basePath%>pages/UserInfo/delete.action?items="+f_str);
      }
      else
      {
        return false;
 ...
 <form id="search" action="pages/UserInfo/list.action" method="post"/>
  ...
  <table class="table_list">
    <tr>
     ...
     <tr>
      <td><input name="item" id="userId=${item.userId}&" type="checkbox" value="userId=${item.userId}&" /></td>
  ...
  <p class="pagination"><!-- pagination start -->
     <emag:paginate_new total="${totalRows}" action="pages/UserInfo/list.action" pageSize="${requestScope.query.pageSize}" pageNumber="${requestScope.query.pageNumber}" />
  </p><!-- pagination end -->
  
  <p class="box_tool"><!-- box_tool start -->
    <a href="pages/UserInfo/create.action" target="_self" class="add" title="新建"><span>新建</span></a>
