...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script>
			function onSelect(event, treeId, treeNode) {
				$('iframe[name="main"]').attr('src','<%=request.getContextPath()%>/system/getSystemMenu.action?menuId=' + treeNode.id);
			}
			$(document).ready(function() {
				...
				$('#doRemove').click(function() {   
					...
					if (window.confirm('该节点的所有子节点将被一起删除，您确定要删除吗？')) {
						var url = '${pageContext.request.contextPath}/system/systemAjax?method=deleteMenusByCode';
						var data = {navId:${param.navId},parentCode : treeNode.code};
				...
				$('#addChild').click(function() {
					var url = '${pageContext.request.contextPath}/system/toAdd?navId=${param.navId}';
					var treeNode = tree.getSelectedNode();
					if (treeNode) {
	...
	<div class="content">
	   <h3 class="breadcrumb">
            <span class="current">您现在的位置：系统管理&gt;<a target="systemMainFrame" href="<%=request.getContextPath()%>/system/queryNavigatorList.action">导航管理</a>&gt;<a target="systemMainFrame" href="<%=request.getContextPath()%>/system/manageSystemMenu.action?navId=${param.navId}">导航菜单设置</a></span>
       </h3>
			...
			<tr>
				<td rowspan="2"  style="padding: 0px 5px 0px 0px;" valign="top" width="200">
					<div  align="left"  style="height:550px;OVERFLOW:auto; width:200px;" class="tree_area" >
					    ...
					    <emag:tree builder="cn.emag.system.web.ajax.MenuSettingTreeBuilder" parameter="${param.navId}" callback="onSelect"/>
					</div>	
				</td>
		    	<td valign="top"><iframe name="main" width="100%" height="550" src="" frameborder="0" scrolling="no" marginwidth="0" valign="top" align="left"></iframe>
				</td>
		  	</tr>
		</table>
	</div>
