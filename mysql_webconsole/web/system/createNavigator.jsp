   ...
   var reg = $("#reg").validate({
	...
	rules: {
	  ...
	  "navigator.navSeq": {
	    digits:true,
	    remote:{
		          url:"<%=request.getContextPath()%>/system/remoteValidateSeq.action",
		          type:"post",
		          datatype:"json",
		          cache:false,
		          data:{
...
<div class="content"><!-- content start -->
   <h3 class="breadcrumb">
   <span class="current">您现在的位置：系统管理&gt;<a target="systemMainFrame" href="<%=basePath%>system/queryNavigatorList.action">导航管理</a>&gt;<a target="systemMainFrame" href="<%=basePath%>system/addNavigator.action">增加导航</a></span>
  </h3>
   
  <h4 class="warning">请填写以下信息完成新建。加“*”为必填项。</h4>
  
  <s2:form id="reg" action="createNavigator">
  <table class="table_form">
    <tr>
      <th>导航名称：</th>
      <td><input name="navigator.navName" id="navigator.navName" type="text" value="" class="required" maxlength="10"/></td>
      <td class="place"><p class="annotation">1-20个字符，只能包括中文字、英文字母、数字和下划线。一个中文为2个字符。</p></td>
    </tr>
    <tr>
      <th>导航链接：</th>
      <td><input name="navigator.navUrl" id="navigator.navUrl" type="text" value="" class="required"/></td>
      <td class="place"><p class="annotation">请填写正确的导航链接地址</p></td>
    </tr>
    <tr>
      <th>导航排序：</th>
      <td><input name="navigator.navSeq" id="navigator.navSeq" type="text" value=""/></td>
      <td class="place"><p class="annotation">请输入新建导航序号，只能为数字，导航显示将按照该排序值的大小从左到右排序</p></td>
    </tr>
    <tr>
      <th>备注：</th>
      <td><input name="navigator.navNote" id="navigator.navNote" type="text" value=""/></td>
      <td class="place"><p class="annotation">请填写备注</p></td>
    </tr>
    <tr>
      <th>导航状态：</th>
      <td>启用：<input type="radio" id="navigator.navStatus" name="navigator.navStatus" value="1" checked/>
           停用：<input type="radio" id="navigator.navStatus" name="navigator.navStatus" value="0"/></td>
