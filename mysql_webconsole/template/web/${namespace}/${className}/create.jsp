...
<html xmlns=" http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>新建<%=${className}.TABLE_ALIAS%></title>
...
<div class="content"><!-- content start -->
   <h3 class="breadcrumb">
   <span class="current">您现在的位置：<%=${className}.TABLE_ALIAS%>管理&gt;<a target="_self" href="${actionBasePath}/list.action"><%=${className}.TABLE_ALIAS%>管理</a>&gt;<a target="_self" href="${actionBasePath}/create.${actionExtension}">新建<%=${className}.TABLE_ALIAS%></a></span>
  </h3>
   
  <h4 class="warning">请填写以下信息完成注册。加“*”为必填项。</h4>
  
  <s:form id="domain" action="${actionBasePath}/save.${actionExtension}" method="post">
  ...
  <table class="table_form">
	...
	<#list table.columns as column>
		...
		<tr>	
			...
			<td>
			<input value="<@jspEl 'model.'+column.columnNameLower/>"  id="${column.columnNameLower}" name="${column.columnNameLower}"  maxlength="${column.size}" class="${column.validateString}" required="${(!column.nullable)?string}"/>
			</td>
			<td class="place"><p class="annotation">提示信息</p></td>
		</tr>
		</#if>
